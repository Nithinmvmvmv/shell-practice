...
Y="\e[33m"
N="\e[0m"
#general log files location
LOGS_FOLDER="/var/log/shellscript-logs"
#if we want to cut the .sh in file name then we use bellow format command
SCRIPT_NAME=$(echo $0 | cut -d "." -f1)
#cretaing log file folder in location
LOG_FILE="$LOGS_FOLDER/$SCRIPT_NAME.log"

#hear,if we use -p for checking the file if not it will create,if alredy exits do nothing(without showing error)
mkdir -p $LOGS_FOLDER
#| tee command for basic log details to see in screen ,and tee command use only like whear the log details is required
